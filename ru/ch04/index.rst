...
.. note::
    ...
    Последняя сборка: |today|

    Исходные коды для этой главы можно найти в `assets каталоге <../../assets>`_.


В этой главе дается обзор QML, декларативного языка пользовательского интерфейса, используемого в Qt 5. Мы обсудим синтаксис QML, который представляет собой дерево элементов, а затем обзор наиболее важных базовых элементов. Позже мы кратко рассмотрим, как создавать свои собственные элементы, называть компоненты и как трансформировать элементы, изменяя их свойства. Ближе к концу мы рассмотрим, как расположить элементы вместе в компоновщике и, наконец, взглянуть на элементы, в которые пользователь может вводить данные.
...
* Оператор ``import`` включает модуль конкретной версии. В общем случае вы всегда будите импортировать *QtQuick 2.0* в качестве исходного набора компонентов
* Комментарии могут быть ``//`` однострочными или ``/* */`` многострочными. Так же как в C/C++ и JavaScript
* Каждый QML файл должен иметь ровно один корневой элемент, как HTML
* Элемент должен быть объявлен с типом и следующими за ним ``{ }``
* Элементы могут иметь свойства, они представлены в виде ``name : value``
* Произвольные элементы внутри QML документа могут быть получены с помощью их ``id`` (идентификатором без кавычек)
* Элементы могут быть вложеными, а это значит, что родительский элементможет иметь дочерние элементы. Доступ к родительскому элементу может быть получен через ключевое слово ``parent``

.. tip::

    Часто нам хочется получить доступ к некоторым элементам по их ``id`` идентификатору или к родительскому элементу используя ключевое слово ``parent``. Хорошей практикой считается использование в качестве имени для корневого элемента "root" ``id: root``. Тогда вам не придется думать о том как назван корневой элемент в вашем QML документе.

.. hint::

    Вы можете запустить пример при помощи Qt Quick из командной строки вашей ОС следующим образом::

        $ $QTDIR/bin/qmlscene RectangleExample.qml

    Здесь надо заменить *$QTDIR* на пкаталог в который установлена Qt. *qmlscene* инициализирует выполнение QtQuick и интерпретирует выбранный QML файл.
